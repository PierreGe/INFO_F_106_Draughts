...
class Draughts(object):
    ...
    def sayWinner(self):
        """Indique le gagnant"""
        return 'Draw game, both players are blocked' if(self.finished == 0) else (nameOf(self.player) + " wins the game !")

    def askEndOfTurn(self):
        """ Permet de savoir si un joueur peut terminer son tour"""
        if self.played:
            self.player = nextPlayer(self.player)
            self.played = False
            self.captured = False
            return True
        return False
    ...
    def prompt(self):
        ...
        if(len(choice) == 1 and ((choice >= 'a' and choice <= 'j') or (choice in ('r', 's', 'l')))):
            if(choice == 'r'):  # <<< Draw
                ...
                print(nameOf(nextPlayer(self.player)), " player, do you accept? ([y]es/[n]o)")
                ...
                if(isDAT(fileName)):
                    ...
                    if(saveAction):
                        try:
                                save(fileName, self.board, self.player)
                                print("file saved.")
                        except IOError:
                            print("I/O Error")
                    elif(loadAction):
                        try:
                            self.player, self.board = load(fileName)
                            print("file loaded.")
