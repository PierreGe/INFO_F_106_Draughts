...
class ArtificialIntelligence(object):
    ...
    def safeMove(self, board, i, j, direction, length):
        """Verifie que la case d'arrivée du mouvement passé en paramètre ne peut
        pas conduire a une capture immédiate par le joueur adverse
        cad verifie que :  sur la nouvelle position il n'y a pas en meme un pion
         adverse dans une des direction et une case libre juste après le pion 
         dans la direction opposée. Si le mouvement passée en paramètre indique
         une prise on decale le tout de 1 dans la direction du mouvement """
        newpos = getNewCoord(i, j, direction, self.player, length)
        destI, destJ = newpos
        capture = False
                ...
                if not outside(board,advI,advJ) and not outside(board,frI,frJ):
                    if nbrFree == 0:
                        if (board[advI][advJ] != 0 and playerColor(board[advI][advJ]) == -self.player) and (board[frI][frJ] == 0 or (frI, frJ) == (i, j)):
                            return False
                    else: # Si le pion adverse est pas sur la case adjacente => verifier si c'est un roi
    ...
    def multiCapt(self,board,i,j):
        """Gère les rafle, continue de faire des prises tant que c'est possible"""
        movementWithCapture = []
    ...
    def findMove(self, board):
        """Renvoi le mouvement choisi selon des caractéristiques précises que 
